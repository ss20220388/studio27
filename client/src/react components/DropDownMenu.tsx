...
const DropDownMenu: React.FC = () => {
    ...
    useEffect(() => {
        ...
        console.log('Current user from cookie (userData):', user)
    ...
    const handleLogout = async () => {
        try {
            await fetch('http://api.studio27.rs/api/auth/logout', { method: 'POST', credentials: 'include' })
        } catch (error) { /* ignore */ }
        setUser(null)
        setOpen(false)
    }
            ...
            {open && (
                <>
                    ...
                    <div className="absolute right-0 mt-3 z-50 w-72 rounded-lg bg-white shadow-lg border border-gray-200 divide-y divide-gray-100">
                        ...
                        <nav className="py-1">
                            <a href="http://app.studio27.rs" className="flex items-center space-x-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                <svg className="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0012 9.75c-2.551 0-5.056.2-7.5.582V21" />
                                </svg>
                                <span>Web aplikacija</span>
                            </a>
                            {user.role === "ROLE_ADMIN" && <a href="http://admin.studio27.rs" className="flex items-center space-x-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                <svg className="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
