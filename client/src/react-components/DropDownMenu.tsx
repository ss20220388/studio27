...
export default function CartModal({ accessToken: initialToken }) {
  ...
  const loginUser = async (email, password) => {
    const res = await fetch(`${API_URL}/api/auth/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include", // Postavlja refreshToken / session cookie u pregledaču
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (regResponse.ok) {
        // 2. Automatsko logovanje odmah nakon registracije
        const loginResponse = await loginUser(payload.email, pass);

        if (loginResponse.ok) {
          // Ako je login vratio token/korisnika, obaveštavamo ostale komponente u aplikaciji (npr. DropDownMenu)
          ...
          // 4. Preusmeravanje na plaćanje sada kada je korisnik sačuvan i autentifikovan
          window.location.href = "/pay";
        } else {
