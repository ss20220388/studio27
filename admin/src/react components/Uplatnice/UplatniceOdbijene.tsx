...
interface UplatniceOdbijeneProps {
  uplatnice: Uplatnica[];
  setSelectedImage: (url: string | null) => void;
   API_URL: string;
}

export default function UplatniceOdbijene({ uplatnice, setSelectedImage,API_URL }: UplatniceOdbijeneProps) {
    ...
    <>
      <div className="mt-12 flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-5 border-b border-neutral-800/60">
        <div>
          ...
          <h2 className="text-2xl font-bold text-red-500">Odbijena plaćanja</h2>
          <p className="text-sm text-neutral-500 mt-1">Pregled odbijenih plaćanja.</p>
        </div>
      </div>
      ...
      <div className="relative max-w-sm" style={{ marginBlock: "10px" }}>
        <input
          type="text"
          placeholder="Pretraži odbijena plaćanja..."
          value={search}
          onChange={(e) => {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {list.length > 0 ? list.map((u, i) => (
                <tr
                  key={u.platioId || `${u.studentId}-${i}` || i + 2000}
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-wider uppercase bg-red-900/20 text-red-400 border border-red-800/30`}
                      >
                        {u.tip || "Nepoznato"}
                      </span>
                      {u.tip === "UPLATNICA" && (u.url || u.urlUplatnice) && (
                        <button
                          ...
                          className="cursor-pointer p-1 text-red-400 hover:bg-red-900/20 rounded-md transition-colors"
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-8 text-center text-neutral-500">
                    Nema odbijenih plaćanja za prikaz.
                  </td>
                </tr>
              )}
            </tbody>
