        ...
        <div className="overflow-x-auto rounded-lg shadow">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {
                    kursevi?.map(kurs => (
                        <tr key={kurs?.id} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700" onClick={()=>{setCurrentKurs(kurs); setActive(!active)}}>
                            ...
                            {active && currentKurs?.id === kurs?.id && (
                                <td className="absolute top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex items-center justify-center">
                                    <div className="bg-white p-6 rounded-lg shadow-lg">
                                        <h3 className="text-lg font-bold mb-2">Lekcije:</h3>
                                        <ul className="list-disc list-inside">
                                            {currentKurs.lekcije?.map((lekcija,index) => (
                                                <li key={index} className="mb-2">
                                                    <h4 className="font-bold">{lekcija?.naziv}</h4>
                                                    <p>{lekcija?.opis}</p> 
                                                    <a href={lekcija?.videoUrl} className="text-blue-500 hover:underline" target="_blank" rel="noopener noreferrer">Pogledaj video</a>
                                                </li>
                                            ))}
                                        </ul>
                                        <button className="mt-4 px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-700" onClick={() => setActive(false)}>Close</button>
                                    </div>
                                </td>
                            )}
                        </tr>
